...
class LogInfo:
    def __init__(self, config: dict, loss: list, ev: list):
        self.config = config
        self.loss = loss
        self.eval = ev
        self.validate()
...
class ModelInfo:
    def __init__(self, path_to_dir: str):
        ...
        self.model_paths = [(os.path.join(path_to_dir, x), int(x.split('.')[0])) for x in os.listdir(path_to_dir) if x.endswith('.nth')]
    ...
    def load_name(self):
        ...
        if config.get('word', False):
            name.append('Word')
        if config.get('w2v_sim'):
            name.append('Sim')
        if config.get('w2v_nn'):
            name.append('NN')
        if self.log.config['distfn'] != 'poincare':
...
def load_all_important_models():
    ...
    for models_dir in os.listdir(root):
        ...
        for model_dir in os.listdir(models_dir):
            ...
            if is_not_important(model_dir):
                continue
            data[model_dir] = ModelInfo(model_dir)
    return data


def load_all_important_log():
    root = '..\\model\\data\\'
    data = {}
